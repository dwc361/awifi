...
public class RoofClassUtils {
	...
	public static Object mapParamsToObject(Map map, Class clazz) throws Exception {
		Object aim = clazz.newInstance();
		Field[] fields = clazz.getDeclaredFields();
		for (int i = 0; i < fields.length; i++) {
			Object val = map.get(fields[i].getName());
			if(val == null){
				continue;
			}
			Class[] clazzArr = { val.getClass() };
			Method method = clazz.getDeclaredMethod("set" + WordUtils.capitalize(fields[i].getName()), clazzArr);// 调用指定属性的get方法
			Object[] argsArr = { val };
			method.invoke(aim, argsArr);
		}
		return aim;
	}
	 ...
	 */
	public static Class<?> loadClass(String dir, String classFullName) throws MalformedURLException,
