package org.roof.commons.create;
...
import org.roof.dataaccess.RoofAbstractDao;
import org.roof.excel.vo.Relation;
...
public class AutoCreateUtils {
	...
	private RoofAbstractDao roofAbstractDao;
	...
	private String templatePrefix = "E:/E/work34/roof-core1.2/src/main/java/org/roof/commons/create/template/";// 文件模版目录
	...
	public String createPoFromTable(String tableName) throws Exception {
		...
		String path = this.getExportPrefix() + SysConstants.PACKAGE_ENTITY + "/"
	...
	public String createVoFromTable(String tableName) throws Exception {
		Domain domain = this.fillDomain(tableName);
		Map<String, Object> root = this.objectParamsToMap(domain);
		String path = this.getExportPrefix() + SysConstants.PACKAGE_VO + "/" + WordUtils.capitalize(domain.getAlias())
				+ "Vo.java";
	...
	public String createXmlFromTable(String tableName) throws Exception {
		...
		String path = this.getExportPrefix() + SysConstants.PACKAGE_DAO + "/" + WordUtils.capitalize(domain.getAlias())
				+ "_exp.xml";
	...
	public String createActionFromTable(String tableName) throws Exception {
		...
		String path = this.getExportPrefix() + SysConstants.PACKAGE_ACTION + "/"
	...
	public String createServiceFromTable(String tableName) throws Exception {
		...
		String path = this.getExportPrefix() + SysConstants.PACKAGE_SERVICE + "/"
	...
	private void addComment(String tableName) {
		try {
			...
			String filename = this.getUserDir() + "/src/main/java/" + this.getPackagePath().replace(".", "/") + "/"
					+ SysConstants.PACKAGE_ENTITY + "/" + WordUtils.capitalize(this.getAliasTable(tableName)) + ".java";
			...
			for (; iter.hasNext();) {
				roofAbstractDao.getSqlMapClientTemplate().queryForList("tool_auto_exp_execute_statement", iter.next());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private List findColumnsFromTable(String tableName) {
		return roofAbstractDao.getSqlMapClientTemplate().queryForList("tool_auto_exp_find_columns_from_table",
				tableName);
	}
	...
	private List findPrimaryKeyFromTable(String tableName) {
		return roofAbstractDao.getSqlMapClientTemplate().queryForList("tool_auto_exp_find_primarykey_from_table",
				tableName);
	}
	...
	private List findRelationFromTable(String tableName) {
		...
		List source = roofAbstractDao.getSqlMapClientTemplate().queryForList("tool_auto_exp_find_relation_from_table",
				args);
	...
	private String findTableDisplayFromTable(String tableName) {
		Object obj = roofAbstractDao.getSqlMapClientTemplate().queryForObject(
	...
	@Resource
	public void setRoofAbstractDao(RoofAbstractDao roofAbstractDao) {
		this.roofAbstractDao = roofAbstractDao;
	}
