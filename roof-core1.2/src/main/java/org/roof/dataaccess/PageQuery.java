...
public class PageQuery {
	...
	private static final String _LAST_ROW_NUM = "lastrownum";

	private static final String _LIMIT = "limit";

	private static final String _FIRST_ROW_NUM = "firstrownum";
	...
	public Page find(Object... values) {
		...
		List<?> rows = daoSupport.findForList(queryStr, page.getStart()
	...
	public Page findByMappedQuery(Object parameterObject) {
		Long rowCount = (Long) daoSupport.executeByMappedQuery(countStr,
				parameterObject);
		page.setTotal(rowCount);
		List<?> rows = daoSupport.findByMappedQuery(queryStr, page.getStart()
				.intValue(), page.getLimit().intValue(), parameterObject);
		page.setDataList(rows);
		return page;
	}
	...
	public Page select(Object parameterObject, boolean autoPage) {
		...
		List<?> rows = null;
		if (autoPage) {
			rowCount = (Long) daoSupport.selectForObject(countStr,
					parameterObject);
			rows = daoSupport.selectForList(queryStr, page.getStart()
					.intValue(), page.getLimit().intValue());
		} else {
			...
			if (ClassUtils.isAssignable(parameterObject.getClass(), Map.class)) {
				paramObj = addMapProperties(parameterObject, firstrownum,
						limit, lastrownum);
			} else {
				paramObj = createParamObj(parameterObject, firstrownum, limit,
						lastrownum);
			}
	...
	private Object addMapProperties(Object parameterObject, long start,
			long limit, long end) {
		Object paramObj;
	...
	private Object createParamObj(Object parameterObject, long start,
			long limit, long end) {
		...
		BeanCopier beanCopier = BeanCopier.create(parameterObject.getClass(),
				o.getClass(), false);
