...
public class SafetyInterceptor extends AbstractInterceptor {
	...
	private static List<String> interceptorRule;// 过滤拦截规则

	private static List<String> legalIp;// 允许访问的IP段

	private static List<String> dynamicToStaticUrlList;// 需要动转静的页面地址

	@Override
	public void init() {
		try {
			if (interceptorRule == null) {
				interceptorRule = PropertiesUtil.initPropList(interceptorRule,
						"interceptorRule");
			}
			if (legalIp == null) {
				legalIp = PropertiesUtil.initPropList(legalIp, "legalIp");
			}
			if (dynamicToStaticUrlList == null) {
				dynamicToStaticUrlList = PropertiesUtil.initPropList(
						dynamicToStaticUrlList, "dynamic_to_static_url");
			}
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
