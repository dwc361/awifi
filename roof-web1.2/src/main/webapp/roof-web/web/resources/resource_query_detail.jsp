...
<script type="text/javascript">
	$(document).ready(
			function() {
				$("#statemet_select_btn").button().click(function(event) {
					$("#statemet_select_dialog").dialog("open");
				});

				$("#statemet_select_dialog").dialog(
						{
							autoOpen : false,
							width : 800,
							height : 600,
							...
							buttons : {
								"确定" : function() {
									$('input[name="privilege.identify"]').val(
											statemet_select_frame.window.getSelectedStatementname()).trigger("change");
									$(this).dialog("close");
								},
								"取消" : function() {
									$(this).dialog("close");
...
<body>
	...
	<form id="mainForm" action="${basePath }/resource_queryAction!update.action" method="post">
		<table border="0" cellpadding="0" cellspacing="1" class="ui-table" width="100%">
			<tr>
				<td class="ui-table-header2" style="text-align: center;" width="20%">过滤名: <input name="privilege.id"
			...
			<tr>
				...
				<td><input type="text" name="privilege.identify" readonly="readonly" style="width: 300px;"
					value="${resource.identify }" />&nbsp;&nbsp;<a id="statemet_select_btn" class="btn" href="#"><i
		...
		<div id="statemet_select_dialog" title="请选择查询">
			<iframe name="statemet_select_frame" src="resource_queryAction!statementSelectPage.action" frameborder="0"
