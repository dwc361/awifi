...
function resetPwd(obj){
	...
	$.ajax({
	    async: false,
	    url : "staffAction!update_pwd.action",
	    data: {"staff.id":obj.id,"staff.password":"${defPwd}"},
	    type: 'post',
	    dataType: 'json',
	    cache: false,
...
<body>
	...
	<form id="mainForm" action="${basePath }/staffAction!list_select.action" method="post">
	...
	<table border="0" cellpadding="0" cellspacing="1" class="ui-table" width="100%">
		...
		<c:forEach var="staff" items="${page.dataList }" varStatus="status">
			<tr>
				...
				<td style="text-align: center; cursor: pointer;" onclick="resetPwd(this)" id="${staff.id }">密码重置</td>
			</tr>
		</c:forEach>
	</table>
