...
public final class SafeDispatcherServlet extends HttpServlet {
	...
	public void service(final ServletRequest req, final ServletResponse resp) throws ServletException, IOException {
		 ...
		 */
		if (this.initSuccess) {
//			String clientUrl = req.getParameter("service");
//			String uri = WebUtils.getRequest().getRequestURI();
//			if ((clientUrl == null) && (pathMatcher.match("/*/login**", uri))) {
//				clientUrl = "/login?service=" + WebUtils.getFullBasePath() + "services/j_acegi_cas_security_check";
//				req.getRequestDispatcher(clientUrl).forward(req, resp);
//				return;
//			}
