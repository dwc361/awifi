...
public class ServiceValidateController extends DelegateController {
	...
	protected final ModelAndView handleRequestInternal(final HttpServletRequest request,
		...
		try {
			...
			Principal mainAcct = assertion.getChainedAuthentications().get(0).getPrincipal();
			String currUser = (String) mainAcct.getAttributes().get("sn");
			currUser = redisMap.get(mainAcct.getId() + "_" + service.getId().split("/")[3] + "_login_subuser");
			String userPwd = (String) mainAcct.getAttributes().get("userPwd");
			userPwd = redisMap.get(mainAcct.getId() + "_" + service.getId().split("/")[3] + "_login_userPwd");
			if (StringUtils.isEmpty(currUser)) {
				Person person = new Person();
				person.setParNode(mainAcct.getId());
				List<Person> subs = ldapUtils.getPersonList(person);
				for (Person p : subs) {
					boolean ok = pathMatcher.match(ObjectUtils.toString(p.getUrl()), service.getId());
					if (ok) {
						currUser = p.getSn();
						break;
					}
			...
			success.addObject("currUser", currUser);
			Long privilege_id = subUserService.selectSubUserByPar(currUser, mainAcct.getId());
			success.addObject("privilege_id", privilege_id);
			if(StringUtils.hasLength(userPwd)) {
				success.addObject("userPwd", ldapUtils.encodeEncryptStringKey(userPwd));
			}
			...
			return generateErrorView(e.getCode(), e.getCode(), new Object[] { serviceTicketId,
