...
import com.zjhcsoft.uac.ldap.util.PersonAttributeMapper;

/**
 * LDAP implementation of
 * {@link org.jasig.services.persondir.IPersonAttributeDao}.
 * 
 * In the case of multi valued attributes a {@link java.util.List} is set as the
 * value.
 * 
 * <br>
 * <br>
 ...
 * <td>
 * Set the {@link SearchControls} used for executing the LDAP query.</td>
 * <td valign="top">No</td>
 ...
 * <td>
 * The base DistinguishedName to use when executing the query filter.</td>
 * <td valign="top">No</td>
 * <td valign="top">""</td>
 * </tr>
 * <tr>
 ...
 * style object that this DAO can retrieve LDAP connections from.</td>
 ...
 * <td>
 * If the ldap attributes set in the ldapAttributesToPortalAttributes Map should
 * be copied into the {@link SearchControls#setReturningAttributes(String[])}.
 * Setting this helps reduce wire traffic of ldap queries.</td>
 * <td valign="top">No</td>
 ...
 * <td>
 * How multiple attributes in a query should be concatenated together. The other
 * option is OR.</td>
 * <td valign="top">No</td>
 ...
 * @since uPortal 2.5
 */
public class LdapPersonAttributeDao extends AbstractQueryPersonAttributeDao<LogicalFilterWrapper> implements
		InitializingBean {
	 ...
	 * appendAttributeToQuery(java.lang.Object, java.lang.String,
	 * java.util.List)
	 */
	@Override
	 ...
	 * @see
	 * org.jasig.services.persondir.support.AbstractQueryPersonAttributeDao#
	 * getPeopleForQuery(java.lang.Object, java.lang.String)
	 */
	@Override
	protected List<IPersonAttributes> getPeopleForQuery(LogicalFilterWrapper queryBuilder, String queryUserName) {
		...
		String clientUrl = WebUtils.getRequest().getParameter("service");
		String subUser = WebUtils.getRequest().getParameter("subUser");
		String userPwd = WebUtils.getRequest().getParameter("userPwd");
		boolean isFilter = (StringUtils.isNotEmpty(clientUrl) ? true : false);
		if (isFilter && (StringUtils.isNotEmpty(subUser))) {
			redisMap.put(queryUserName + "_" + substring(clientUrl.split("/")[3]) + "_login_subuser", subUser);
			redisMap.put(queryUserName + "_" + substring(clientUrl.split("/")[3]) + "_login_userPwd", userPwd);
			List<IPersonAttributes> peopleAttributes = new ArrayList<IPersonAttributes>(1);
			Map<String, List<Object>> queryResult = new HashMap<String, List<Object>>();

			// Person person = new Person();
			// person.setCn(subUser);
			// person.setParNode(queryUserName);
			// person = ldapUtils.getPersonList(person).get(0);
			// String[] fields = PersonAttributeMapper.fields;
			// for (int i = 0; i < fields.length; i++) {
			// try {
			// Class[] clazzArr = null;
			// Method method = Person.class.getDeclaredMethod("get" +
			// RoofStringUtils.firstUpperCase(fields[i]),
			// clazzArr);
			// Object[] argsArr = null;
			...
			// valList.add(val);
			// queryResult.put(fields[i], valList);
			// }
			// } catch (Exception e) {
			// e.printStackTrace();
			// }
			...
			snList.add(subUser);
			queryResult.put("sn", snList);
			List<Object> valList = new ArrayList<Object>();
			valList.add(userPwd);
			queryResult.put("pwd", valList);
			peopleAttributes.add(new CaseInsensitiveNamedPersonImpl(queryUserName, queryResult));
	 ...
	 * @deprecated Set the property on the {@link SearchControls} and set that
	 *             via {@link #setSearchControls(SearchControls)}
	 */
	@Deprecated
	...
	/**
	 * Optional wrapper template for the generated part of the query. Use {0} as
	 * a placeholder for where the generated query should be inserted.
	 */
